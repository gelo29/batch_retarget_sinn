...
def rotate_hips(rootOffset,rotOffset1, rotOffset2):
    for c in components:
        if(bool(re.match(pattern, c.Name)) == False and c.Name != 'pelvis'):
            ...
            c.Selected = True
            trans_curr_value = c.Translation
            trans = FBVector3d(trans_curr_value[0]+float(rotOffset1),trans_curr_value[2],trans_curr_value[1]+float(rotOffset2))
            c.GetVector(trans,FBModelTransformationType.kModelTranslation, True)
            rot_curr_value = c.Rotation
            rot = FBVector3d(-90 + (float(rot_curr_value[0]+float(rootOffset)) * -1), 0, 90)
            scale = FBVector3d(1,1,1)
...
def run_program():
    ...
    csv_file_path = "C:/Users/AVA_ME_TL/Documents/Gelo/Export/batch/Sheet.csv"#os.path.join(root_dir, config_dict["csv_file_name"])
    rigged_character = "C:/Users/AVA_ME_TL/Documents/Gelo/Export/batch/Target.fbx"#os.path.join(root_dir, "Target.fbx")
    ...
    with open(csv_file_path, 'r') as _filehandler:
        ...
        for row in readCSV:
            app.FileNew()
            fbx_file = "C:/Users/AVA_ME_TL/Documents/Gelo/Export/batch/"+row[0].strip()+".fbx"#os.path.join(root_dir, config_dict["read_dir"], row[0].strip() + ".fbx")
            ...
            if success_open:
                ...
                plotTo("skel","60")
                if len(row) > 6:
                    print(row[4].strip(),row[5].strip(),row[6].strip())
                    rotate_hips(row[4].strip(),row[5].strip(),row[6].strip())
                
                FBSystem().CurrentTake.LocalTimeSpan = FBTimeSpan(
                    FBTime(0, 0, 0, int(row[1].strip()), 0),
                    FBTime(0, 0, 0, int(row[2].strip()), 0)
                )
